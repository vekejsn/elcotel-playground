...
ENUM_RATE_VALS = {254: "Unlim.", 255: "Restr."}
...
def build_decompressed(
    ...
    rate_entries: list,
    nxx_table: list,
    intrastate_npas: list,
    surcharges: dict,
    npa_prices: list | None = None,
) -> bytearray:
    """
    ...
    # The first 800 bytes are the default NPA table for area codes 200-999.
    if npa_prices:
        for entry in npa_prices:
            npa = entry.get("npa")
            raw_value = entry.get("raw_value")
            if npa is None or raw_value is None:
                continue
            if 200 <= npa <= 999:
    ...
    # These metadata pointers are written by the original editor and are
    # constant across the corpus: 0x0377 marks the counts block and 0x037A
    # points at the fixed group-header start.
    decomp[873] = 0x77
    decomp[874] = 0x03
    decomp[875] = 0x7A
    decomp[876] = 0x03
    ...
    for i in range(8):
        ...
        decomp[800 + i + 48] = surch.get("spare_1", 0)
        decomp[800 + i + 56] = surch.get("spare_2", 0)
